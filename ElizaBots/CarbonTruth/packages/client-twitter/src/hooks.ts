  ...
  static async fetchWebImage(
    ...
    tweetInfo: TweetInfo
      ...
      try {
        ...
        if (!selectedImageUrl) {
          elizaLogger.log("No new image found, using the image generation.");
          selectedImageUrl = await this.generateImage(
            runtime,
            tweetText,
            tweetInfo,
          );
        ...
        elizaLogger.error("Error posting tweet with Image:", error);
        return "NO_IMAGE";
    }
  }
